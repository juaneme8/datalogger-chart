...
export default function Chart({ chartData }:any) {
	useEffect(() => {
		...
		for (let i = 1; i < chartData.length-1; i++) {
			...
			const date = new Date(chartData[i].data[0]*1000);
			...
			data.push({
				date: formattedTime,
				[chartData[0].data[20]]: chartData[i].data[20],
			});
		}
	...
	return (
		<>{chartData.length > 0 ? <div id='chartdiv' style={{ width: '100%', height: '500px' }}></div> : <h4>No se ha cargado ningún archivo</h4>}</>
	);
}